...
impl ToolInvoker {
    ...
    pub fn from_toolchain_dir(
        ...
        .map(|invoker| Self::from_external_invoker(invoker, toolchain_dir))
    ...
    pub fn from_tool_and_toolchain_dir(tool_exe: &Path, toolchain_dir: &Path) -> Self {
        Self::from_external_invoker(ExternalInvoker::from_executable(tool_exe), toolchain_dir)
    }

    fn from_external_invoker(mut invoker: ExternalInvoker, toolchain_dir: &Path) -> Self {
    ...
    pub fn configure_command_for_toolchain(toolchain_dir: &Path, command: &mut Command) {
